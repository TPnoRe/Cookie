...
class TapEngine:
    ...
    def tap(self, x_pct, y_pct, hold_ms=None):
        ...
        jitter_px = float(s.get('click_jitter_px', 1.0))
        ...
        if hold_ms is None:
            raw_hold = float(s.get('click_hold', 0.05))
    ...
    def tap_px(self, x, y, hold_ms=0):
        ...
        width, height = size
        s = self._settings()
        jit = float(s.get('click_jitter_px', 1.0))
        ...
        hold_sec = max(0.05, float(hold_ms) / 1000.0 if hold_ms > 0 else 0.05)
