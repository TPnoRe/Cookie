...
def purchase_desired_random_boost(desired_template, desired_name, logger=None):
    def say(message):
        if logger is not None:
            logger(message)
        print(message)

    # ── ตรวจ boost ปัจจุบันก่อน ──────────────────────────────────────────────
    #say(f"🔍 ตรวจสอบบัฟปัจจุบันก่อนสุ่ม: {desired_name}...")
    current_screen = device_capture_screen(config.DEVICE_IP, config.DEVICE_PORT)
    if current_screen is not None and detect_templates(current_screen, desired_template, RANDOM_BOOST_REGION):
        say(f"✅ เจอบัฟ '{desired_name}' เปิดอยู่แล้ว -> ข้ามการสุ่ม (ประหยัดเหรียญ)")
        return
    say(f"🔄 ยังไม่เจอบัฟ '{desired_name}' -> เริ่มสุ่มซื้อ...")
    # ── ซื้อและ roll ───────────────────────────────────────────────────────────
    say("🛒 กำลังกดซื้อ Random Boost...")
    safe_device_tap(config.DEVICE_IP, config.DEVICE_PORT, RANDOM_BOOST_ITEM[0], RANDOM_BOOST_ITEM[1])
    time.sleep(random.uniform(0.8, 1.4))
    ...
    time.sleep(random.uniform(0.8, 1.4))
    #say(f"🔍 กำลังรอตรวจจับบัฟที่ต้องการ: {desired_name}...")
    timeout = 30
    start_time = time.time()
    roll_attempt = 0
    while True:
        if time.time() - start_time > timeout:
            say(f"⏰ หมดเวลา {timeout} วิ: ยังไม่เจอบัฟ '{desired_name}' หลังสุ่ม")
            say("⚠️ ข้าม Desired Random Boost. กรุณาตรวจสอบค่าในเกมว่าตั้งไว้ถูกต้อง")
            return
        ...
        if detect_templates(screen, desired_template, RANDOM_BOOST_REGION):
            if roll_attempt > 0:
                say(f"✅ เจอบัฟที่ต้องการแล้ว: {desired_name}!")
            else:
                say(f"✅ เจอบัฟที่ต้องการแล้ว: {desired_name}!")
            break
        time.sleep(0.5)
        ...
        if roll_attempt % 10 == 0:
            say(f"⏳ สุ่มไปแล้ว {roll_attempt * 0.5:.0f} วิ ยังไม่เจอบัฟ '{desired_name}'...")
