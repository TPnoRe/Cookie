...
def get_round_screenshot(round_num: int, type: Optional[str] = "result"):
    data = bot_engine.round_screenshots.get(round_num)
    jpeg_bytes = None
    if isinstance(data, dict):
        if type and type in data and data[type]:
            jpeg_bytes = data[type]
        elif type == "box_after" and data.get("box_after"):
            jpeg_bytes = data["box_after"]
        elif type == "box_before" and data.get("box_before"):
            jpeg_bytes = data["box_before"]
        elif type == "result" and data.get("result"):
            jpeg_bytes = data["result"]
        elif data.get("box_after"):
            jpeg_bytes = data["box_after"]
        elif data.get("box_before"):
            jpeg_bytes = data["box_before"]
    ...
    type_names = {
        "result": "ผลลัพธ์จริง (คะแนน / เหรียญ / EXP)",
        "box_before": "กล่องปริศนา (ก่อนกด Open all)",
        "box_after": "ของรางวัล (หลังกด Open all)",
    }
    type_label = type_names.get(type, "ภาพหน้าจอผลลัพธ์")
    blank = np.zeros((480, 854, 3), dtype=np.uint8)
    blank[:] = (20, 24, 35)
    cv2.putText(blank, f"Round #{round_num}: {type_label}", (160, 220), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 200, 100), 2, cv2.LINE_AA)
    cv2.putText(blank, "No screenshot available for this step", (220, 260), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (180, 180, 190), 1, cv2.LINE_AA)
    _, buf = cv2.imencode(".jpg", blank)
    return Response(content=buf.tobytes(), media_type="image/jpeg")


@app.get("/api/round-screenshot-info/{round_num}")
def get_round_screenshot_info(round_num: int):
    data = bot_engine.round_screenshots.get(round_num)
    if isinstance(data, dict):
        return {
            "has_result": bool(data.get("result")),
            "has_box_before": bool(data.get("box_before")),
            "has_box_after": bool(data.get("box_after")),
        }
    return {
...
def api_check_deps():
    return dep_check.check_installed()


@app.get("/loader", response_class=HTMLResponse)
def get_loader():
    loader_path = os.path.join(os.path.dirname(__file__), "loader.html")
    if os.path.isfile(loader_path):
        with open(loader_path, "r", encoding="utf-8") as f:
            return f.read()
    return "<h1>Loader page not found</h1>"
